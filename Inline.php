...
namespace Magentoid\Component\Yaml;

use Magentoid\Component\Yaml\Exception\DumpException;
use Magentoid\Component\Yaml\Exception\ParseException;
use Magentoid\Component\Yaml\Tag\TaggedValue;
...
class Inline
{
     ...
     * @return mixed
     *
     * @throws ParseException|\Exception when object parsing support was disabled and the parser detected a PHP object or when a reference could not be resolved
     */
                ...
                switch (true) {
                    ...
                    case 0 === strpos($scalar, '!php/object'):
                        if (self::$objectSupport) {
                            if (!isset($scalar[12])) {
                                self::trigger_deprecation('1.1', 'Using the !php/object tag without a value is deprecated.');
                    ...
                    case 0 === strpos($scalar, '!php/const'):
                        if (self::$constantSupport) {
                            if (!isset($scalar[11])) {
                                self::trigger_deprecation('1.1', 'Using the !php/const tag without a value is deprecated.');
                ...
                switch (true) {
                    case ctype_digit($scalar):
                        if (preg_match('/^0[0-7]+$/', $scalar)) {
                            self::trigger_deprecation('Magentoid/yaml', '1.1', 'Support for parsing numbers prefixed with 0 as octal numbers. They will be parsed as strings as of 6.0. Use "%s" to represent the octal number.', '0o'.substr($scalar, 1));
                    ...
                    case '-' === $scalar[0] && ctype_digit(substr($scalar, 1)):
                        if (preg_match('/^-0[0-7]+$/', $scalar)) {
                            self::trigger_deprecation('1.1', 'Support for parsing numbers prefixed with 0 as octal numbers. They will be parsed as strings as of 6.0. Use "%s" to represent the octal number.', '-0o' . substr($scalar, 2));
...
EOF;
    ...
    private static function trigger_deprecation(string $string1, string $string2, string $string3)
    {
